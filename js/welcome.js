...
$.ajax({
  ...
  data: {
    key: "HYVBZ-DFF34-6DLUN-KDQGA-VKQLJ-HVBMX",
    output: "jsonp",
  },
  dataType: "jsonp",
...
function showWelcome() {
  ...
  switch (ipLocation.result.ad_info.nation) {
    ...
    case "中国":
      pos =
        ...
        " " +
        ipLocation.result.ad_info.district;
  ...
  try {
    ...
    document.getElementById(
      "welcome-info"
    ).innerHTML = `你好啊，来自 <b><span style="color: var(--kouseki-ip-color);font-size: var(--kouseki-gl-size)">${pos}</span></b> 的小友💖<br>${posdesc}🍂<br>当前位置距博主约 <b><span style="color: var(--kouseki-ip-color)">${dist}</span></b> 公里！<br>您的IP地址为：<b><span>${ip}</span></b><br>${timeChange}`;
  } catch (err) {
